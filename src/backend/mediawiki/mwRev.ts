...
import { ScriptJobInput, ScriptJobCoordinator, ScriptJob } from '../util/scriptJobs.ts';
...
import { MwRevEntity } from './mwDbInterface.ts';
...
export async function getAndStoreRevisions(mwClient: MwClientInterface, titleOrId: string|number, asyncLog: AsyncLog): Promise<{
  ...
  if (await mwClient.db.hasRevision(mwPage.lastrevid)) {
    ...
    return { page: mwPage, revisions: await mwClient.db.fetchRevisions(mwPage.pageid) };
  }
  ...
  // rev id -> revision:
  const savedRevs: Record<number, MwRevision> = await mwClient.db.getSavedRevisions(revisions.map(r => r.revid));
...
export async function computeRevSegments(client: MwClientInterface, allRevs: MwRevision[], args: ScriptJobInput<'mwRevSave'>, asyncLog: AsyncLog): Promise<MwOwnSegmentHolder> {
  ...
  if (!args.resegment && allRevs.every(rev => !!rev.segments)) {
    await asyncLog(`All revisions already have segments computed, nothing left to do for this job.`);
    segmentHolder.setSegments(allRevs[allRevs.length - 1].segments);
    return segmentHolder;
  }

  await asyncLog(`Computing revision segments...`);

  let revNum = 1;
  for (let rev of allRevs) {
    try {
      if (revNum % 25 === 0 || revNum === allRevs.length) {
        await asyncLog(`Computing revision segments... (${revNum}/${allRevs.length})`);
      }
  ...
  await asyncLog(`Finished computing revision segments; ${updateBatch.length} revisions needed computing, ` +
    `${allRevs.length - updateBatch.length} revisions already had segments computed.`);

  if (updateBatch.length) {
    await asyncLog(`Saving revision segments to database.`);
    ...
    await asyncLog(`Finished saving revision segments to database.`);
  } else {
    await asyncLog(`No new revision segments to save to database.`);
  }
